...
import 'package:tm_college_app/widgets/home_screen/home_screen_body_homeworks.dart';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        HomeScreenBodyAverages(
            ...
            onTapFunctionGradeCard: ({
              ...
              Navigator.push(
                context,
                MaterialPageRoute(
                  settings: RouteSettings(arguments: [subject, index]),
                  builder: (_) => ViewAverageScreen(
                    database: widget.database,
                    sharedPreferences: widget.sharedPreferences,
                    // subject: subject,
                    // index: index,
      ...
      appBar: ModularAppBar(
        ...
        actions: currentPageIndex == 0
            ? [
                ...
                //       ],
                //     );
                //     Navigator.pushNamed(context, "/settings_screen").then((_) {
                //       if (scrollControllerHomeworks.hasClients) {
                //         double offset = scrollControllerHomeworks.offset;
                //         scrollControllerHomeworks.animateTo(
                //           offset + 0.5,
      ...
      floatingActionButton: ModularFloatingActionButton(
        onPressedFunction: () => _routePointer(currentPageIndex),
        icon: Icons.add_rounded,
      ),
      bottomNavigationBar: NavigationBar(
        onDestinationSelected: (int destination) {
          //DefaultTabController.of(context)!.animateTo(destination);
          setState(() => currentPageIndex = destination);
        },
